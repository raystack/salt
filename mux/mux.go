...
func (mux *cmuxWrapper) shutDown() error {
	mux.grpcServer.GracefulStop()

	shutdownCtx, cancel := context.WithTimeout(context.Background(), mux.gracePeriod)
	defer cancel()

	return mux.httpServer.Shutdown(shutdownCtx)
}
